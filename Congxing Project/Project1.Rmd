...
Differ <- TukeyHSD(x=aov(Data1$WFT~Data1$Tre), 'Data1$Tre', conf.level=0.95)#Tukey test to compute the differences between each two groups 
Differ
library(multcomp)
cld(summary(glht(m1, linfct = mcp(Tre = "Tukey"))),decreasing = T)#Tukey test to attribute them to different groups
```
...
with(Data1,HSD.test(WFT,Tre,df,MSerror, group=TRUE,console=TRUE,main="Difference between the  treatments"))#Another way to do Tukey test
```
...
```{r}
m2 <- glm(cbind(WFT,Num-WFT)~Tre-1,family = quasibinomial(link = "logit"),data = Data1)#Fit the data into a quasibinomial model(overdispersion)
summary(m2)
library(multcomp)
...
```{r}
library(gamlss)
m3 <- gamlss(cbind(WFT, Num-WFT) ~ Tre-1, data=Data1, family=BB)#Fit the data into a beta binomial model(solve the problem of overdispersion in binomial model)
summary(m3)
```

```{r}
library(mgcv)
m4 <- gam(list(WFT ~ Tre - 1, ~Tre), family = gaulss(link = list("identity", "logb")),data = Data1)#Another method to deal with outlier talked in class(change the model)
summary(m4)
```
